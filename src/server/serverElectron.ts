...
// Electron production server - security enabled (TLS + auth token)
try {
    let customPort = parseInt(process.env.AZURE_IOT_EXPLORER_PORT); // tslint:disable-line:radix
    if (isNaN(customPort)) {
        customPort = SERVER_PORT;
    }
    serverInstance = new ServerBase(customPort || SERVER_PORT, true);
    serverInstance.init();
}
catch {
    serverInstance = new ServerBase(SERVER_PORT, true);
    serverInstance.init();
}
